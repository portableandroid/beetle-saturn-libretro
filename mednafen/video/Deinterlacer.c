...
/*
 * Saturn-specific notes:
 *
 *   The Saturn VDP2 renderer always writes at native resolution
 *   directly into the libretro surface, so the "upscale shift"
 *   value (s) that the PSX deinterlacer carries to handle PSX's
 *   internal upscaler is hard-coded to 0 here. Every `<< s` in
 *   the port below becomes `<< 0` (no-op), every `1u << s` becomes
 *   1, every `(... << s) * pitch_pix` becomes `... * pitch_pix`.
 *   The structure is preserved unchanged so future upstream merges
 *   from beetle-psx-libretro stay mechanical: only the `s` source
 *   would need replacement to add upscaling support.
 */
#define SATURN_UPSCALE_SHIFT 0
 ...
 *
 * Backported from beetle-psx-libretro's mednafen/video/Deinterlacer.c,
 * which in turn ported PCSX2's CPU FastMAD in
 * pcsx2/GS/Renderers/SW/GSDeviceSW.cpp -- itself a CPU port of
 * interlace.glsl's ps_main3 + ps_main4 (MAD_BUFFER / MAD_RECONSTRUCT
 * shader passes).
 *
 ...
 *
 * Upscaling:
 *   Saturn's VDP2 SW renderer writes at native resolution; the
 *   upscale shift `s` reaches us as 0 from Process via
 *   SATURN_UPSCALE_SHIFT, so `upscale == 1` is always taken in
 *   the branches below.  Structure preserved from upstream to
...
{
   const unsigned s          = SATURN_UPSCALE_SHIFT;
